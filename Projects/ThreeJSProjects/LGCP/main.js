...
// GAME
  ...
  object = obj;
    ...
    if(selObj)
          ...
          //Rolling
          if(keyName == "r" && playerID == gSnap.val().turn && ((pInfo.curMoves == pInfo.moveAmt && !newGameRef) || (newGameRef)))
          {
            if(newGameRef)
            {
          ...
          if((keyName == "ArrowUp" || keyName == "ArrowDown" || keyName == "ArrowRight" || keyName == "ArrowLeft"))
          {
            ...
            if(curP.val().curMoves < curP.val().moveAmt)
            {
              ...
              if(dir != -1 && mainPieces[curP.val().loc][dir + parseInt(mainPieces[curP.val().loc][4] == "'" ? 6 : 8)] != -1)// && mainPieces[pInfo.loc][dir + 6 + (mainPieces[pInfo.loc][4] == "'" ? 0 : 3)] != -1)
              {
                ...
                //mainPieces[curP.val().loc].occ -= 1;
                  var loc = parseInt(mainPieces[curP.val().loc][dir + parseInt(mainPieces[curP.val().loc][4] == "'" ? 6 : 8)]); // + 6
                  //mainPieces[curP.val().loc].occ += 1;

                  /*if(gSnap.child('player' + i).child('loc').exists())
                  {
                    for(i = 0; i < gSnap.val().playerAmt; i++)
                    {
                        if(i != playerID && gSnap.child('player' + i).val().loc == loc)
                        {
                          a.push(loc);
                          console.log("other added : " + i);
                        }
                        console.log("added : " + i);
                    }
                  }*/
                ...
                //player.position.set(parseInt(mainPieces[pInfo.loc][2]),parseInt(mainPieces[pInfo.loc][3]) + 16,parseInt(mainPieces[pInfo.loc][4]));
                  ...
                  if(mainPieces[loc][mainPieces[loc][4] == "'" ? 11 : 13] && gSnap.child('player' + turn).val().curMoves == gSnap.child('player' + turn).val().moveAmt)
                  {
                      document.getElementById("l_moveAmt").innerHTML = gSnap.child('player' + turn).val().moveAmt.toString() + " + 1";
                      newGameRef.child('player' + turn).update({
                          moveAmt: gSnap.child('player' + turn).val().moveAmt + 1
                      });
                  }
                  else
                  {
                  ...
                  //playersArr[turn].position.y = (16 * mainPieces[curP.val().loc].occ);
...
//rotateAroundWorldAxis(go, new THREE.Vector3(0,0,1), parseFloat(prop[4]) * Math.PI/180);
              ...
              if(prop[11 + 100])
              {
                  var co = usableColors[parseInt(prop[(alt ? 14 : 12)])];
                  console.log(co);
                  console.log(co.color);
                  console.log(prop[(alt ? 14 : 12)]);
                  var subitem = new THREE.Mesh(player.geometry,new THREE.MeshPhongMaterial({envMap: cubeMap,color: parseInt(co.color),transparent: (co.op < 1),opacity: co.op,reflectivity: (co.reflect ? 1 : 0.55),specular: 0xaaaaaa,shininess: 10}));
                  subitem.position.set(go.position.x,go.position.y + 16,go.position.z);
                  subitem.scale.set(5,5,5);
                  scene.add(subitem);
...
firebase.database().ref('gameplay/Bot/player0/').on('value',function(snap){
  if(snap.exists() && playersArr[0] && pieces[snap.val().loc]){
      var tempPos = playersArr[0].position;
      //for(i = 0; i <)
      ...
      /*if(gSnap.child('player' + i).child('loc').exists())
      {
        for(i = 0; i < gSnap.val().playerAmt; i++)
        {
            if(i != playerID && gSnap.child('player' + i).val().loc == snap.val().loc) a.push(gSnap.child('player' + i).val().loc);
        }
      }*/
      playersArr[0].position.set(pieces[snap.val().loc].position.x,pieces[snap.val().loc].position.y + (16 * (mainPieces[snap.val().loc][mainPieces[snap.val().loc][4] == "'" ? 11 : 13] ? 7 : 1)),pieces[snap.val().loc].position.z); // * (a.length + 1)
      if(snap.val().dir) playersArr[0].rotation.set(0,(snap.val().dir == 0 ? 0 : snap.val().dir == 1 ? 180 : snap.val().dir == 2 ? -90 : snap.val().dir == 3 ? 90 : 0) * Math.PI / 180,0);
      //if(tempPos != playersArr[0].position)

      /*if(mainPieces[snap.val()][mainPieces[snap.val()][4] == "'" ? 11 : 13] && gSnap.val().player0.curMoves + 1 == gSnap.val().player0.moveAmt)
      {
          document.getElementById("l_moveAmt").innerHTML = gSnap.val().player0.moveAmt.toString() + " + 1";
          newGameRef.child('player0').update({
              moveAmt: gSnap.val().player0.moveAmt + 1
          });
      }*/
      //alert("stop");
      //console.log("stop : " + snap.parent.val());
      //console.log(snap.parent.val().loc);

  }
});
firebase.database().ref('gameplay/Bot/player1/').on('value',function(snap){

  if(snap.exists() && playersArr[1] && pieces[snap.val().loc]){
      var tempPos = playersArr[1].position;
      ...
      playersArr[1].position.set(pieces[snap.val().loc].position.x,pieces[snap.val().loc].position.y + (16 * (mainPieces[snap.val().loc][mainPieces[snap.val().loc][4] == "'" ? 11 : 13] ? 7 : 1) * (a.length + 1)),pieces[snap.val().loc].position.z);
      if(snap.val().dir) playersArr[1].rotation.set(0,(snap.val().dir == 0 ? 0 : snap.val().dir == 1 ? 180 : snap.val().dir == 2 ? -90 : snap.val().dir == 3 ? 90 : 0) * Math.PI / 180,0);


  /*if(snap.exists() && playersArr[1] && pieces[snap.val()]){
      var tempPos = playersArr[1].position;
      playersArr[1].position.set(pieces[snap.val()].position.x,pieces[snap.val()].position.y + (16 * (mainPieces[snap.val()][mainPieces[snap.val()][4] == "'" ? 11 : 13] ? 6 : 1)),pieces[snap.val()].position.z);
      if(gSnap.child('player1').child('dir').val()) playersArr[1].rotation.set(0,(gSnap.child('player1').val().dir == 0 ? 0 : gSnap.child('player1').val().dir == 1 ? 180 : gSnap.child('player1').val().dir == 2 ? -90 : gSnap.child('player1').val().dir == 3 ? 90 : 0) * Math.PI / 180,0);
...
*/
      ...
      //if(tempPos != playersArr[1].position)

      /*if(mainPieces[snap.val()][mainPieces[snap.val()][4] == "'" ? 11 : 13])
      {
          document.getElementById("l_moveAmt").innerHTML = gSnap.val().player1.moveAmt.toString() + " + 1";
          newGameRef.child('player1').update({
              moveAmt: gSnap.val().player1.moveAmt + 1
          });
      }*/
