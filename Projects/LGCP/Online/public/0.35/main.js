...
document.addEventListener('keydown',function(e){
  ...
  if(isSpace && selObjs.length == 1)
  {
  	...
  	AddPiece(0,{x: arrP.props[1] + (dir == 3 ? 30 : dir == 2 ? -30 : 0), y: arrP.props[2] + (dir == 1 ? -30 : dir == 0 ? 30 : 0), color: 0, spaced: true, dir: dir});
  }
  else
  {
    ...
    selObjs.forEach(function(c){
    	...
    	if(dir == 3 || dir == 2)
      {
        ...
        lGameRef.child("pieces/" + r).update({
        	x: (lSnap.child("pieces/" + r).val().props[1] + (dir == 3 ? speed : -speed))
        });
      }
      if(dir == 0 || dir == 1)
      {
        ...
        //par.style.marginTop = (parseFloat(par.style.marginTop) + (dir == 1 ? -16 : 16)) + "px";
       	lGameRef.child("pieces/" + r).update({
        	y: (lSnap.child("pieces/" + r).val().props[2] + (dir == 1 ? -speed : speed))
        });
      }
      c.id = old;
   	});
...
var res2;
  ...
  lGameRef.child("pieces/" + i).on('value',function(snap){
    ...
    document.getElementById(who).parentNode.style.marginLeft = (GetPos(1,snap.val().props)/* + (lo.val().props[4] != "'" || lo.child("props/rot").exists() ? 0 : 0)) */+ "px";
    document.getElementById(who).parentNode.style.marginTop = (GetPos(2,snap.val().props) + (lo.val().props[4] != "'" || lo.child("props/rot").exists() ? 0 : 0)) + "px";
    //Set Rotation
  ...
  var lx = parseFloat(prop[1]); //(o.off ? o.off.left : 0)
  lGameRef.child("pieces/" + i).update({x: lx}); //parseFloat(prop[1]) * 1.5 + 500 + (o.off ? o.off.left : 0) + (prop[4] != "'" ? -2 : 0) + "px";
}
...
function CreateGameP2(va)
{
  ...
  for(var i = 0; i < res.length; i++)
  {
    ...
    pieces[i].x = 0;
    pieces[i].y = 0;
    //pieces[i].x = GetPos(1,prop,o); //parseFloat(prop[1]) * 1.5 + 500 + (o ? (o.off ? o.off.left : 0) : 0) + (prop[4] != "'" ? -2 : 0);
    //pieces[i].y = GetPos(2,prop,o); //parseFloat(prop[2]) * 1.5 + 250 + (o ? (o.off ? o.off.top : 0) : 0) + (prop[4] != "'" ? -2 : 0);
  }
...
function GetPos(num,prop)
{
  ...
  if(num == 1)
  {
    return parseFloat(prop[1]) * 1.5 + 500 + (o ? (o.off ? o.off.left : 0) : 0) + (prop[4] != "'" ? -2 : 0);
  }
  else if(num == 2)
  {
    return parseFloat(prop[2]) * 1.5 + 250 + (o ? (o.off ? o.off.top : 0) : 0) + (prop[4] != "'" ? -2 : 0);
  }
}
