...
function DestroyPiece(alts)
{
  ...
  var ob = scene.getObjectByName("ph_" + i);
  var inde = pieces.indexOf(ob);
  pieces.splice(inde,1);
  scene.remove(ob);
  console.log(t);
  console.log(i);
  //console.log(t);
...
{
console.log(id);
	if(!atr) atr = {x: 10, y: 10, color: 0, spaced: false};
  //if(!id) id = 5;
  console.log(id);
  ...
  if(id.split(".")[0] == "0")
  {
    src = [id,0,0,0,"'",0,-1,-1,-1,-1,lSnap.val().pieces.length];
  }
  else
  {
    src = [id,0,0,-4,"'",0];
